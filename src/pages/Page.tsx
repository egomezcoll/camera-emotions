...
const Page: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const perf = (msg) =>
      (dom.perf.innerText =
        "tensors:" +
        human.tf.memory().numTensors.toString() +
        " | performance: " +
        JSON.stringify(msg).replace(/"|{|}/g, "").replace(/,/g, " | ")); // print performance element
    async function detectionLoop() {
      ...
      if (!dom.video.paused) {
        ...
        fps.detectFPS =
          Math.round((1000 * 1000) / (human.now() - timestamp.detect)) / 1000;
        fps.frames++;
        fps.averageMs =
          Math.round((1000 * (human.now() - timestamp.start)) / fps.frames) /
          1000;
    ...
    async function webCam() {
      ...
      const webcamStatus = await human.webcam.start({
        element: dom.video,
        crop: false,
        width: 640,
        id,
      }); // use human webcam helper methods and associate webcam stream with a dom element
      log(webcamStatus);
